...
class PanelModoModel extends Model {
	...
	public function users($p, $limit) {
		...
		$users->sanctions = Utils::executeRequest(self::$bdd, "SELECT SQL_CALC_FOUND_ROWS s.type, s.user, s.date, s.reason, u.name FROM hd_user_sanctions AS s INNER JOIN hd_user_users AS u ON s.moderator = u.id");
